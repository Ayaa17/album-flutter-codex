...
class ActivityRepository {
  ...
  Future<Activity> quickCapture({required String defaultName}) async {
    ...
    final existing = activities.where((item) => item.name == defaultName).toList();
    final target = existing.isNotEmpty ? existing.first : await createActivity(defaultName);
  ...
  Future<Activity> renameActivity({required String id, required String newName}) async {
    final directory = await _storageService.ensureActivityDirectory(id);
    ...
    if (await file.exists()) {
      try {
        ...
        return _ActivityMetadata(
          ...
          createdAt: createdAtString == null ? DateTime.now() : DateTime.parse(createdAtString),
        );
      } catch (_) {
  ...
  Future<void> _writeMetadata(Directory directory, _ActivityMetadata metadata) async {
    final file = File(p.join(directory.path, _metadataFileName));
